...
public class Methods {
    ...
    public String getAttribute(By by, String attributeName){return  findElement(by).getAttribute(attributeName);}
    ...
    public void randomElementClick (By by){
        List<WebElement> productElems = driver.findElements(by);
        int maxProducts = productElems.size();
        Random random = new Random();
        int randomProdcut = random.nextInt(maxProducts);
        productElems.get(randomProdcut).click();

    }
    public boolean stokKontrol(By by, String att){
        String attribute = getAttribute(by,att);
        String text="disabled";
        if (attribute.toLowerCase().contains(text.toLowerCase())){
